...
Requires: equity_curves__4_.csv, drivers_timeseries__2_.csv in same directory
...
TRADING_DAYS = 252
COLORS = {
    "static": "#6366f1",       # indigo
    "steadyguard": "#10b981",  # emerald
    "static_fill": "rgba(99,102,241,0.15)",
    "sg_fill": "rgba(16,185,129,0.15)",
    "grid": "#1e293b",
    "bg": "#0a0e17",
    "paper": "#0a0e17",
    "text": "#94a3b8",
...
def load_data():
    ...
    ec = pd.read_csv(ec_path, parse_dates=["date"])
    dr = pd.read_csv(dr_path, parse_dates=["date"])

    # Filter to signal period
    ec = ec[ec["date"] >= "2007-01-31"].copy().reset_index(drop=True)
    ec["spy_ret"] = ec["SPY_BH"].pct_change()
    ec["ret_6040"] = ec["60_40"].pct_change()
    ec["month"] = ec["date"].dt.to_period("M")

    # Extract TLT returns from 60/40 via weight-drift decomposition
    tlt_rets = []
    w_s, w_t = 0.6, 0.4
    prev_m = None
    for _, row in ec.iterrows():
        s, p, m = row["spy_ret"], row["ret_6040"], row["month"]
        if pd.isna(s) or pd.isna(p):
            tlt_rets.append(np.nan); prev_m = m; continue
        if m != prev_m: w_s, w_t = 0.6, 0.4
        tlt_r = (p - w_s * s) / w_t if w_t > 0.01 else np.nan
        tlt_rets.append(tlt_r)
        if (1 + p) != 0: w_s = w_s * (1 + s) / (1 + p); w_t = 1 - w_s
        prev_m = m
    ec["tlt_ret"] = tlt_rets
    ...
    state_lookup = dict(zip(dr["month"], dr["risk_state"]))

    # Regime counts
    regime_counts = dr["risk_state"].value_counts().to_dict()

    return ec[["date", "month", "spy_ret", "tlt_ret"]].copy(), scalar_lookup, state_lookup, regime_counts


def run_backtest(data, scalars, eq_pct):
    """Run static vs SteadyGuard backtest."""
    eq = eq_pct / 100.0
    bd = 1.0 - eq
    ...
    sg_eq = eq * sc
    sg_ws, sg_wt = sg_eq, 1.0 - sg_eq

    for i in range(1, len(data)):
        row = data.iloc[i]
        sr, tr, m = row["spy_ret"], row["tlt_ret"], row["month"]

        if pd.isna(sr) or pd.isna(tr):
            static_c.append(static_c[-1]); sg_c.append(sg_c[-1])
            prev_m = m; continue
...
def calc_metrics(curve, dates):
    """CAGR, MaxDD, Vol, Sharpe, Calmar, Total Return."""
    arr = np.array(curve)
...
# ── Load Data ──────────────────────────────────────────────────────────
data, scalars, states, regime_counts = load_data()
...
with col_info:
    ...
    <div style="background:#111827; border:1px solid #1e293b; border-radius:10px;
        ...
        <div style="font-size:0.8rem; color:#64748b; margin-top:0.2rem;">{n_years:.1f} years · {len(data):,} trading days</div>
...
# ── Metric Cards ───────────────────────────────────────────────────────
def delta_str(sv, sgv, fmt="pct", lower_better=False):
    d = sgv - sv
    better = d < 0 if lower_better else d > 0
    # For MaxDD, less negative = better, so sgv > sv means better
    if fmt == "pct":
...
<div class="metric-row">
    <div class="metric-card highlight">
        ...
        {delta_str(sm['CAGR'], sgm['CAGR'])}
    </div>
    <div class="metric-card highlight">
        <div class="metric-label">Max Drawdown</div>
        <div class="metric-value {'green' if sgm['Max Drawdown'] > sm['Max Drawdown'] else 'red'}">{sgm['Max Drawdown']:.2%}</div>
        {delta_str(sm['Max Drawdown'], sgm['Max Drawdown'])}
    </div>
    <div class="metric-card highlight">
        <div class="metric-label">Sharpe Ratio</div>
        <div class="metric-value green">{sgm['Sharpe']:.2f}</div>
        {delta_str(sm['Sharpe'], sgm['Sharpe'], fmt="num")}
    </div>
    <div class="metric-card">
        <div class="metric-label">Total Return</div>
        <div class="metric-value">{sgm['Total Return']:.0%}</div>
        {delta_str(sm['Total Return'], sgm['Total Return'])}
    </div>
</div>
""", unsafe_allow_html=True)
...
fig.add_trace(go.Scatter(
    x=results["date"], y=results["static"], name=f"Static {equity_pct}/{bond_pct}",
    line=dict(color=COLORS["static"], width=1.8), opacity=0.8,
), row=1, col=1)

fig.add_trace(go.Scatter(
    x=results["date"], y=results["steadyguard"], name=f"SteadyGuard {equity_pct}/{bond_pct}",
    line=dict(color=COLORS["steadyguard"], width=2), opacity=0.95,
), row=1, col=1)

# Drawdown
for label, col, color, fill in [
    (f"Static DD", "static", COLORS["static"], COLORS["static_fill"]),
    (f"SteadyGuard DD", "steadyguard", COLORS["steadyguard"], COLORS["sg_fill"]),
]:
    curve = results[col].values
    peak = np.maximum.accumulate(curve)
    dd = (curve - peak) / peak
    fig.add_trace(go.Scatter(
        x=results["date"], y=dd, name=label,
        line=dict(color=color, width=0.8), fill="tozeroy",
...
fig.update_yaxes(type="log", title="Growth of $1", gridcolor=COLORS["grid"],
                 gridwidth=0.5, row=1, col=1, tickprefix="$", tickformat=".1f")
fig.update_yaxes(title="Drawdown", gridcolor=COLORS["grid"], gridwidth=0.5,
...
metrics_list = [
    ("CAGR", "CAGR", "pct", False),
    ("Max Drawdown", "Max Drawdown", "pct", True),
    ("Annualized Vol", "Volatility", "pct", True),
    ("Sharpe Ratio", "Sharpe", "num", False),
    ("Calmar Ratio", "Calmar", "num", False),
    ("Total Return", "Total Return", "pct", False),
]
...
for label, key, fmt, lower_better in metrics_list:
    ...
    if fmt == "pct":
        sv_s = f"{sv:.2%}"
        sgv_s = f"{sgv:.2%}"
        d_s = f"{'+'if d>0 else ''}{d:.2%}"
    else:
        sv_s = f"{sv:.2f}"
        sgv_s = f"{sgv:.2f}"
        d_s = f"{'+'if d>0 else ''}{d:.2f}"

    # Determine winner
    if lower_better:
        sg_wins = sgv < sv  # for vol: lower is better
        if key == "Max Drawdown":
            sg_wins = sgv > sv  # less negative = better
    else:
        sg_wins = sgv > sv

    sg_cls = "win" if sg_wins else ""
    st_cls = "" if sg_wins else "win"
    d_cls = "win" if sg_wins else "lose"

    table_rows += f"""
    <tr>
        <td>{label}</td>
        <td class="{st_cls}">{sv_s}</td>
        <td class="{sg_cls}">{sgv_s}</td>
        <td class="{d_cls}">{d_s}</td>
    </tr>"""
...
<table class="comp-table">
    <thead>
        <tr>
            <th>Metric</th>
            <th>Static {equity_pct}/{bond_pct}</th>
            <th>SteadyGuard</th>
            <th>Delta</th>
        </tr>
    </thead>
